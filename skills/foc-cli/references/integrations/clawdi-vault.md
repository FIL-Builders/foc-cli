...
Pin `--keyProject` whenever the account has more than one project. Never copy a config between machines expecting the reference to mean the same thing.
...
| Works in your shell, fails under an agent or MCP | Different PATH or a different default project for that process. Pin `--keyProject`. |
